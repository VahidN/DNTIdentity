...
public static class CustomDataProtectionExtensions
{
    ...
    private static X509Certificate2 LoadCertificateFromFile(SiteSettings siteSettings)
    {
        ...
        using (var store = new X509Store(StoreName.My, StoreLocation.CurrentUser))
        {
            ...
            using var x509Certificate2 = X509CertificateLoader.LoadPkcs12FromFile(fileName, certificate.Password,
                X509KeyStorageFlags.Exportable);

            store.Add(x509Certificate2);
        }

        var cert = X509CertificateLoader.LoadPkcs12FromFile(fileName, certificate.Password,
